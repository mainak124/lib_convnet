...
#ifndef _inference_poolOut1_4_H_
#define _inference_poolOut1_4_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "inference_poolOut1_4_memcore.h"

SC_MODULE(inference_poolOut1_4) {
...
protected:
    ...
    // buffer instances
    inference_poolOut1_4_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(inference_poolOut1_4) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "inference_poolOut1_4_memcore_U_";
            name += szTmp;

            buffer[i] = new inference_poolOut1_4_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "inference_poolOut1_4.trace");
            // ports
